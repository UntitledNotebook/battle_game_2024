...
Sprint::Sprint(GameCore *game_core, uint32_t id, uint32_t player_id)
...
void Sprint::TankMove(float move_speed, float rotate_angular_speed) {
  ...
  if (player) {
    ...
    if (input_data.key_down[GLFW_KEY_LEFT_CONTROL] && sprint_ > 0.001f) {
      offset.y *= sprint_scale_;
      sprint_ -= kSecondPerTick / sprint_time_;
    }
    if (!input_data.key_down[GLFW_KEY_LEFT_CONTROL]) {
      sprint_ = std::min(sprint_ + kSecondPerTick / sprint_recover_time_, 1.0f);
    }
...
void Sprint::RenderLifeBar() {
  if (lifebar_display_) {
    ...
    pos = parent_unit->GetPosition() + sprintbar_offset_;
    shift = {(float)lifebar_length_ * (1 - sprint_) / 2, 0.0f};
    SetTransformation(pos - shift, 0.0f, {lifebar_length_ * sprint_, 1.0f});
    SetColor(sprintbar_front_color_);
    DrawModel(life_bar_model_index);
  }
}

void Sprint::TurretRotate() {
  auto player = game_core_->GetPlayer(player_id_);
  if (player) {
    auto &input_data = player->GetInputData();
...
bool Sprint::IsHit(glm::vec2 position) const {
